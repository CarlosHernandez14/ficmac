...
const CardMedico = ( { medico } ) => {
  return (
    <div className='w-full bg-[#A0737D] shadow-xl rounded-xl px-[3%] py-[2%]'>
        <div className="flex justify-start items-center">
            <span className=' flex justify-center items-center text-white w-10'>
            ...
            {/* Nombre medico */}
            <h1 className="text-white text-2xl font-semibold ml-4">{medico.nombre_completo}</h1>
        </div>

        {/* Datos del medico */}
        <div className="flex flex-col justify-start items-start mt-4">
            <p className="text-white text-lg">Ubicación: {medico.rfc}</p>
            <p className="text-white text-lg">Especialidad: {medico.especialidad}</p>
            <p className="text-white text-lg">Correo: {medico.matricula}</p>
            <p className="text-white text-lg">Teléfono: {medico.num_celular}</p>
        </div>
        ...
        <div className="flex justify-start items-center space-x-4">
            <button className="bg-white text-[#A0737D] rounded-lg px-4 py-2">Eliminar</button>
        </div>
    </div>
  )
}
